...
import tqdm
root='/VOC2012'
save_root='Downloads/data/VOC20122'
if not os.path.exists(save_root):
    os.mkdir(save_root)
id='2' #2或者7，为了给后面生成的文件做标记  2007：7 2012:2
split='train'  #用于标记是train还是val
root_txt='VOC2012/ImageSets/Main/val.txt'
...
with open(root_txt,'r') as f:
    ...
    while True:
        ...
        if show:
            print('old_image  ',old_image,os.path.exists(old_image))
            save_show_root='Downloads/data/test_crop_show'
            im=cv2.imread('Downloads/data/voc/VOCtrainval_11-May-2012/VOCdevkit/VOC2012/JPEGImages/2008_000008.jpg')
            print(im.size)
            if not os.path.exists(save_show_root):
                os.mkdir(save_show_root)
        ...
        for whole_image_ob_idx,ob in enumerate(objects):  #图中的每个instance都会当一次中心处理点
            ...
            for cur_ob_pathch,sub_split_ans in enumerate(split_ans): #以当前instance为中心处理点，会产生几个子图
                ...
                if show:
                    ...
                    show_tuple.append(image_sub)
        # 第二种方法，调用我们自己编写的一个函数实现任意大小、任意通道数的图像堆叠显示
        # 以原来的0.5倍并排显示两张图片
        # stackedimageh = ManyImgs(0.2, show_tuple)

        # 垂直显示
        # stackedimagev = ManyImgs(0.2, ([imgflower, imgwomen], [imgwomen, imgflower]))

        # 如果只有图片数量为奇数，并希望能够垂直显示，可以创建一个空白图像
        Blankimg = np.zeros((800, 800), np.uint8)  # 大小可以任意函数会将其强制转换
        stackedimageb = ManyImgs(0.5, show_tuple)
